...
private:
	...
	int recursiveFind(const std::string& pName, const std::string& startPath,
		std::ostream& outStream, int& matches) const;

	std::shared_ptr<FSNode> copySubTreeHelper(std::shared_ptr<FSNode> rootPtr) const;
	std::shared_ptr<FSNode> copyHelper(std::string pName, const std::string& startPath, 
